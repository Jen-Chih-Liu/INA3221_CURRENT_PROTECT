MCU Firmware Specification (v1.5)
# M251 GPU Cable Current Protection Firmware

版本: v1.5
日期: 2026-03-30
修訂: 修改異常觸發後的告警行為：STATE_LATCHED（3 分鐘鎖定後）不再維持 LED 常亮 / Buzzer 靜音，改為 LED 與 Buzzer 持續以 2 Hz 同步閃爍警告，直到裝置斷電重置為止。此變更確保操作人員即使在鎖定後仍能持續收到視覺與聽覺警示。
...
- **Imbalance Debounce**: 連續 `IMBALANCE_DEBOUNCE_COUNT = 3` 次採樣超標才觸發，實際防彈跳時間 = 3 × 1700 ms = **5.1 s**，避免負載動態變化誤報。
- **Overcurrent Debounce**: 連續 `OVERCURRENT_DEBOUNCE_COUNT = 3` 次採樣任意通道 > 9300（93A）才觸發，實際防彈跳時間 = 3 × 1700 ms = **5.1 s**。
