...
- 需連續 **5 次**採樣皆超標（`IMBALANCE_DEBOUNCE_COUNT = 5`）才確認異常。
  - 每次採樣間隔由 `TIMER_MONITOR_UPDATE = 1700 ms` 控制，實際防彈跳時間約 5 × 1700 ms = **8.5 秒**。
...
- **Overcurrent Debounce**: 連續 `OVERCURRENT_DEBOUNCE_COUNT = 5` 次採樣任意通道 > OC 閾值才觸發，實際防彈跳時間 = 5 × 1700 ms = **8.5 s**。閾值可透過 `UPOC` 指令於執行期更新，預設 **9200 mA**。
