MCU Firmware Specification (v2.2)
...
### D. Protection 3: Zero-Current — 單線斷流保護

**觸發條件:**
- 任意通道電流 = **0 A**（`currents[i] == 0`）。
- 需連續 **5 次**採樣皆偵測到零電流（`ZERO_CURRENT_DEBOUNCE_COUNT = 5`）才確認異常。
  - 每次採樣間隔由 `TIMER_MONITOR_UPDATE = 200 ms` 控制，實際防彈跳時間約 5 × 200 ms = **1 s**。
  - `g_u8ProtectionChecked` 旗標確保每個採樣週期僅評估一次，避免 main loop 重複累加計數。
- Status Register **Bit 6** (`STATUS_BIT_ZERO_CURRENT`) 置 1。
- **Per-Channel Alert**: 每個週期同步更新 `0x2E`（`I2C_REG_ZERO_CURRENT_ALERT_CH`）。任一通道電流為 0 時對應 bit 置 1（與 debounce 狀態無關，反映當下即時狀態）。
- 確認後觸發與 Protection 1、2 相同的**統一三段式告警狀態機**（Phase 1 → Phase 2 → Latch）。

---

### E. Protection 4: Undercurrent — 低電流保護 ⚠️ 已停用
...
**Protection 1（電流不平衡）、Protection 2（過電流）、Protection 3（單線斷流）確認後均觸發此告警狀態機。任一異常確認後立即啟動 180 秒倒數。**
...
- `Protection_Handler` 主迴圈偵測到旗標後：
  1. 若尚未儲存 Log：**將系統狀態設為 `STATE_WARN`**，並執行 `Save_Log_Entry()` 儲存快照至 EEPROM 1。
  2. 清除 `g_u8HwWarningTriggered` 旗標。
  > **注意**：Log 由 `g_bLogSavedForLatch` 保護，僅儲存一次。`Read_Monitor_Data_0()` / `Read_Monitor_Data_1()` 的即時重讀已注解，使用上一次採樣資料。PS_PGOOD 控制行亦已注解，目前**不驅動**。
- **`Warning_Indication_Task()` 負責驅動 `STATE_WARN` 狀態的告警指示**（LED/Buzzer/N_PLAY 均不在 `Protection_Handler` 直接操作）：
  - **非 NSP23 板**：LED 以 2 Hz（250ms ON / 250ms OFF）持續閃爍，Buzzer 同步開關。
  - **NSP23 板**：重複設定 `g_u8NP23Play2Flag = 1`，主迴圈在 I2C1 空閒時重複呼叫 `N_PLAY(2)`。
- `STATE_WARN` **不進入倒數狀態機**，不需斷電恢復（與 `STATE_LATCHED` 不同）。
...
INA_WARNING Falling Edge 中斷
  ...
  g_u8HwWarningTriggered = 0
        │
        ▼  Warning_Indication_Task() (每次主迴圈呼叫)
  STATE_WARN分支：
  LED: 2 Hz 閃爍（250ms ON / 250ms OFF）
  BUZZER: 2 Hz 開關（非 NSP23 板）
  NSP23 板：重複觸發 N_PLAY(2)
```
...
| `0x2E` | 1 B | Zero-Current Alert Channel | R | **Zero-Current Per-Channel Alert Bitmap**（見 3.2）；任一通道電流 = 0 A 時對應 bit 置 1，每監測週期更新 |
...
|-----|------|------|
| 7 | Reserved | 保留 |
| 6 | `STATUS_BIT_ZERO_CURRENT` | 任一通道電流 = 0 A，debounce 確認後置 1 |
...
- **`0x2E` (Zero-Current Alert CH)**：某通道電流 = 0 A 時，對應 bit = `1`
...
- **Zero-Current Debounce（v2.0 新增）**: 任一通道電流 = 0 A 連續 `ZERO_CURRENT_DEBOUNCE_COUNT = 5` 次（約 1 s）觸發保護，Status Register Bit 6（`STATUS_BIT_ZERO_CURRENT`）置 1，並啟動統一三段式告警狀態機。`0x2E`（`I2C_REG_ZERO_CURRENT_ALERT_CH`）per-channel bitmap 每週期無條件更新。
...
- **Per-Channel Alert Registers（v1.8/v2.0 新增）**: `0x2C`（OC Alert CH）、`0x2D`（Imbalance Alert CH）與 `0x2E`（Zero-Current Alert CH）每個監測週期（200 ms）由 `Protection_Handler()` 無條件更新，不受 debounce 計數影響，反映當下即時狀態。Host 可先讀 `0x0E` Status Register 確認告警類型，再讀對應 bitmap 取得 channel 詳細資訊。三個暫存器僅存在於 SRAM（`eeprom_ram`），**不**持久化至 Flash EEPROM。
...
- **STATE_WARN（v2.1 新增）**: HW Warning（`INA_WARNING` GPIO 中斷）觸發後系統進入 `STATE_WARN`。此狀態**不啟動倒數狀態機**，不需斷電恢復。`Warning_Indication_Task()` 在 `STATE_WARN` 分支驅動 LED 2 Hz 閃爍 + Buzzer（非 NSP23 板）或重複 N_PLAY(2)（NSP23 板）。Log 由 `g_bLogSavedForLatch` 保護儲存一次。
- **Warning_Indication_Task() 狀態分支概覽**: 此函數負責全部 LED/Buzzer/NSP23 輸出：倒數期間（`STATE_WARNING_COUNTDOWN`）驅動 Phase 1/2；`STATE_LATCHED` 後驅動持續 2 Hz + NSP23 N_PLAY(2)；`STATE_WARN` 分支驅動持續 2 Hz + NSP23 N_PLAY(2)。SysTick ISR 僅負責倒數計時與 MCU 運行時間，**不**直接操作 GPIO 或 NSP23 旗標。
...
- **NSP23 音效 IC（選配）**: 韌體於 `I2C1_Init()` 中讀取 NSP23 PID 並存入 `NP23_pid`；若等於 `0xACCCCAAA`，`Warning_Indication_Task()` 於主迴圈中設定 `g_u8NP23PlayFlag` / `g_u8NP23Play2Flag` 旗標，主迴圈在 I2C1 空閒（`g_u8GetEndFlag_0 == 1`）時呼叫 `N_PLAY(1)` / `N_PLAY(2)` 取代蜂鳴器動作。`STATE_LATCHED` 與 `STATE_WARN` 後 `Warning_Indication_Task()` 都會持續設定 `g_u8NP23Play2Flag = 1`，主迴圈即時重複播放 `N_PLAY(2)`。LED/Buzzer GPIO 與 NSP23 旗標**全部由 `Warning_Indication_Task()` 驅動**，`Protection_Handler` 中的鬼嘖指示程式碼已改為 `#if 0`。SysTick ISR 僅負責倒數計時與 MCU 運行時間。NSP23_RST（PB.14）在初始化時拉 High 保持 IC 正常運作。
