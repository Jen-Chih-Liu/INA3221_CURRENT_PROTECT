...
版本: v1.6
日期: 2026-03-30
修訂: 新增低電流（Undercurrent）保護功能。當任一通道電流低於可配置閾值（預設 500 mA）並連續 5 次採樣確認後，觸發與過電流相同的三階段告警序列（LED→蜂鳴器→鎖定）。同時，過電流與低電流閾值由原先的編譯期常數改為可透過 I2C 指令（`UPOC` / `UPUC`）動態調整並持久化儲存於 EEPROM，不再需要重新燒錄韌體。
...
| `PS_PGOOD`      | `PF.3`   | 電源好信號輸出（Normal=high, Protection=low）|
...
| PS_PGOOD        | Output     | low         | 保護信號（Normal=high, Protection=low）          |
...
- 需連續 **3 次**採樣皆超標（`IMBALANCE_DEBOUNCE_COUNT = 3`）才確認異常。
  - 每次採樣間隔由 `TIMER_MONITOR_UPDATE = 1700 ms` 控制，實際防彈跳時間為 3 × 200 ms = **0.6 秒**。
...
**觸發條件:**
- 任意通道電流 > **OC 閾值**（預設 **9300 mA = 9.3 A**，可透過 `UPOC` 指令動態調整）。
- 需連續 **5 次**採樣皆超標（`OVERCURRENT_DEBOUNCE_COUNT = 5`）才確認異常。
  - 每次採樣間隔由 `TIMER_MONITOR_UPDATE = 1700 ms` 控制，實際防彈跳時間約 **8.5 s**。
  - `g_u8ProtectionChecked` 旗標確保每個採樣週期僅評估一次，避免 main loop 重複累加計數。
- Status Register **Bit 5** (`STATUS_BIT_OVERCURRENT`) 置 1。
- 閾值儲存於 EEPROM 1 Offset `0x28–0x2B`，斷電後保留設定值。
...
- 閾值儲存於 EEPROM 1 Offset `0x2C–0x2F`，斷電後保留設定值。
...
**Protection 1、2、3 均使用相同的告警狀態機。任一異常確認後立即啟動 180 秒倒數。**
...
| `0x28–0x2B` | 4 B | OC Threshold | R/W | 過電流閾值（mA），預設 **9300**，指令 `UPOC` 更新 (LSB first) |
| `0x2C–0x2F` | 4 B | UC Threshold | R/W | 低電流閾值（mA），預設 **500**，指令 `UPUC` 更新 (LSB first) |
| **即時監測數據區** | | | | |
...
|-----|------|------|
| 7 | Reserved | 保留 |
| 6 | `STATUS_BIT_UNDERCURRENT` | 任一通道 < UC 閾值（預設 500 mA），debounce 確認後置 1 |
...
- 更新後立即生效，並持久化儲存至 EEPROM 1 Offset `0x28–0x2B`
...
- B0–B3: 新閾值（mA），32-bit Little-Endian
- 更新後立即生效，並持久化儲存至 EEPROM 1 Offset `0x2C–0x2F`
...
| `0x28–0x2B`    | OC_THRESHOLD         | 4 B    | 過電流閾值（mA），預設 **9300**，指令 `UPOC` 更新 |
| `0x2C–0x2F`    | UC_THRESHOLD         | 4 B    | 低電流閾值（mA），預設 **500**，指令 `UPUC` 更新 |
...
- **Imbalance Debounce**: 連續 `IMBALANCE_DEBOUNCE_COUNT = 5` 次採樣超標才觸發，實際防彈跳時間 = 5 × 1700 ms = **8.5 s**，避免負載動態變化誤報。
- **Overcurrent Debounce**: 連續 `OVERCURRENT_DEBOUNCE_COUNT = 5` 次採樣任意通道 > OC 閾值才觸發，實際防彈跳時間 = 5 × 1700 ms = **8.5 s**。閾值可透過 `UPOC` 指令於執行期更新，預設 **9300 mA**。
- **Undercurrent Debounce**: 連續 `UNDERCURRENT_DEBOUNCE_COUNT = 5` 次採樣任意通道 < UC 閾值才觸發，實際防彈跳時間 = 5 × 1700 ms = **8.5 s**。閾值可透過 `UPUC` 指令於執行期更新，預設 **500 mA**。觸發後與過電流採用相同的三階段告警序列（LED→蜂鳴器→鎖定）。
- **可配置閾值持久化**: OC / UC 閾值寫入 EEPROM 1（Offset `0x28` / `0x2C`），重新上電後自動載入，不需重新燒錄韌體。
- **Debounce 計時機制（v1.4 修正）**: `Protection_Handler()` 透過 `g_u8ProtectionChecked` 旗標，確保防彈跳計數器每個 `TIMER_MONITOR_UPDATE`（1700 ms）週期只遞增一次。`u8MonitorFlag` 觸發時重設旗標，I2C 傳輸完成後執行一次保護評估，後續 main loop 迴圈不再重複計數。
