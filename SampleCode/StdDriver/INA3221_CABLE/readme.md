MCU Firmware Specification (v1.9)
...
- 主迴圈偵測旗標後，使用**最後一次採樣的現有資料**（`eeprom_ram` 中的快照）直接執行 Event Log 儲存（`Save_Log_Entry()`）。
  > **注意**：`Read_Monitor_Data_0()` / `Read_Monitor_Data_1()` 的即時重讀呼叫已在程式碼中注解，HW Warning 觸發時不會重新讀取感測器，使用上一次 `TIMER_MONITOR_UPDATE`（200 ms）週期的資料。
- 執行 Event Log 快照（`Save_Log_Entry()`，寫入 **EEPROM 1**，eeprom_system，0xD800）。
- PS_PGOOD 拉 **High**（保護動作）。
- ⚠️ **注意**：目前韌體**未**將 `STATUS_BIT_HW_WARNING`（Bit 4）置 1，亦不進入 `STATE_LATCHED` 狀態機；保護動作為直接拉高 PS_PGOOD 並儲存 Log 各一次。
...
| `0x2E–0x2F` | 2 B | Reserved | — | 保留（原 UC Threshold 欄位，已停用） |
| **即時監測數據區** | | | | |
...
### 3.2 Alert Channel Registers (0x2C / 0x2D) Bit Definition

兩個暫存器共用相同的 bit 格式，每個 bit 對應一個監測通道：
...
- 兩個暫存器每個監測週期（200 ms）**無條件更新**，與 debounce 計數無關
- 建議讀取流程：先讀 `0x0E`（Status Register）確認 Bit3/Bit5，再讀 `0x2D`/`0x2C` 取得 Channel 詳細資訊
...
| `0x2E–0x2F`    | Reserved             | 2 B    | 保留（原 UC_THRESHOLD 停用欄位） |
...
- **Per-Channel Alert Registers（v1.8 新增）**: `0x2C`（OC Alert CH）與 `0x2D`（Imbalance Alert CH）每個監測週期（200 ms）由 `Protection_Handler()` 無條件更新，不受 debounce 計數影響，反映當下即時狀態。Host 可先讀 `0x0E` Status Register 確認告警類型，再讀對應 bitmap 取得 channel 詳細資訊。兩個暫存器僅存在於 SRAM（`eeprom_ram`），**不**持久化至 Flash EEPROM。
- **可配置閾值持久化**: OC 閾值寫入 EEPROM 2（Offset `0x28`），重新上電後自動載入，不需重新燒錄韌體。UC 閾值欄位（原 Offset `0x2C`，現已縮減為 `0x2E`）已停用，目前不被 MCU 讀寫。
...
- **NSP23 音效 IC（選配）**: 韌體於 `I2C1_Init()` 中讀取 NSP23 PID 並存入 `NP23_pid`；若等於 `0xACCCCAAA`，`Warning_Indication_Task()` 於主迴圈中設定 `g_u8NP23PlayFlag` / `g_u8NP23Play2Flag` 旗標，主迴圈在 I2C1 空閒（`g_u8GetEndFlag_0 == 1`）時呼叫 `N_PLAY(1)` / `N_PLAY(2)` 取代蜂鳴器動作。STATE_LATCHED 後不再觸發 N_PLAY，改由 `Warning_Indication_Task()`（主迴圈）直接驅動 LED 2 Hz 閃爍（非 NSP23 板則同時驅動蜂鳴器）。SysTick ISR 僅負責倒數計時與 MCU 運行時間，**不**直接操作 LED/Buzzer GPIO 或 NSP23 旗標。NSP23_RST（PB.14）在初始化時拉 High 保持 IC 正常運作。
