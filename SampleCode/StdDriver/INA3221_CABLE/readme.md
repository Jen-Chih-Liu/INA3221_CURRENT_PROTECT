MCU Firmware Specification (v1.8)
...
- 主迴圈偵測旗標後，立即呼叫 `Read_Monitor_Data_0()` / `Read_Monitor_Data_1()` 取得最新感測資料快照。
...
- **NSP23 音效 IC（選配）**: 韌體於 `I2C1_Init()` 中讀取 NSP23 PID 並存入 `NP23_pid`；若等於 `0xACCCCAAA`，主迴圈在 I2C1 空閒（`g_u8GetEndFlag_0 == 1`）時呼叫 `N_PLAY(1)` / `N_PLAY(2)` 取代蜂鳴器動作。STATE_LATCHED 後不再觸發 N_PLAY，改由 SysTick 直接驅動 LED 2 Hz 閃爍（非 NSP23 板則同時驅動蜂鳴器）。NSP23_RST（PB.14）在初始化時拉 High 保持 IC 正常運作。
