...
void SysTick_Handler(void)
{
    ...
    /* --- Three-phase warning sequence: Phase1(0-20s) -> Phase2(20s-3min) ->
     * Latch --- */
    if (g_u32WarningCountdownMs > 0)
    {
        ...
        if (g_u32WarningCountdownMs == 0)
        {
            ...
            /* LED and Buzzer keep blinking - handled by STATE_LATCHED block on next
             * tick */
        }
        else if ((LATCH_COUNTDOWN_MS - g_u32WarningCountdownMs) <
                 BUZZER_DELAY_MS)
        {
    ...
    // --- MCU Runtime Counter ---
    // Counts milliseconds; wraps to seconds and writes to eeprom_ram for host
    // read-back.
    ...
    if (g_u16McuRunTimeMillis >= 1000)
    {
        ...
        // Use a direct 32-bit write to the aligned address to avoid memcpy warnings
        // with volatile. The cast to a volatile pointer ensures the compiler
        // handles the memory access correctly.
        *((volatile uint32_t *)&eeprom_ram[I2C_REG_OFFSET_MCU_RUN_TIME]) =
            g_u32McuRunTimeSeconds;
    }
}
        ...
        if (g_system_state != STATE_LATCHED)
        {
            ...
            // if(min_current!=0)
            {
                ...
                if ((max_current - min_current) >
                        (g_AppConfig.u32IMBALANCE_THRESHOLD / 10))
                {
            ...
            //    IMBALANCE_ALERT_CH (0x2D): bit i set when channel deviates from mean
            //    > imbalance_thresh/2
            {
...
void Event_Log_Handler(void)
{
    ...
    if (u8UPITFlag == 1)
    {
        ...
        Write_EEPROM(&eeprom_user, EE_OFFSET_IMBALANCE_THRESHOLD,
    ...
    if (u8UPOCFlag == 1)
    {
        ...
        Write_EEPROM(&eeprom_user, EE_OFFSET_OC_THRESHOLD,
    ...
    if (u8UPCDFlag == 1)
    {
        ...
        Write_EEPROM(&eeprom_user, EE_OFFSET_COUNTDOWN,
    ...
    if (u8UPDCFlag == 1)
    {
        ...
        Write_EEPROM(&eeprom_user, EE_OFFSET_SW_DEBOUNCE, &g_AppConfig.u8swdebounce,
                     sizeof(uint8_t));
    }
    ...
    if (u8UPMFFlag == 1)
    {
        ...
        Write_EEPROM(&eeprom_user, EE_OFFSET_MFG_DATE, g_au8MFGDate,
                     EEPROM_MFG_DATE_SIZE);
    }
    ...
    if (u8UPLTFlag == 1)
    {
        ...
        /* UPLT: persist lot ID */
        Write_EEPROM(&eeprom_user, EE_OFFSET_LOT_ID, g_au8LotID,
                     EEPROM_LOT_ID_SIZE);
    }
    ...
    if (u8UPSNFlag == 1)
    {
        ...
        Write_EEPROM(&eeprom_user, EE_OFFSET_SERIAL_NUMBER, g_au8SerialNumber,
                     EEPROM_SERIAL_NUMBER_SIZE);
    }
}
/*---------------------------------------------------------------------------------------------------------*/
