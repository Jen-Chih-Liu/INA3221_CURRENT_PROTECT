...
#define EEPROM_BLOCK_SIZE   (5 * 1024)    /* Each emulated EEPROM partition = 5 KB       */
...
static enum SystemState g_system_state         = STATE_NORMAL; /* Fault state machine: NORMAL / WARNING_COUNTDOWN / LATCHED */
static uint8_t  g_u8ImbalanceDebounceCounter   = 0;            /* Current-imbalance fault debounce counter                  */
static uint8_t  g_u8OcDebounceCounter          = 0;            /* Overcurrent (OC) fault debounce counter                   */
static uint8_t  g_u8UcDebounceCounter          = 0;            /* Undercurrent (UC) debounce counter (reserved)             */
volatile uint32_t g_u32WarningCountdownMs       = 0;            /* Warning countdown timer in ms; decremented by SysTick     */
static uint8_t  g_bLogSavedForLatch            = 0;            /* Guards against saving the event log more than once        */
static volatile uint8_t g_u8HwWarningTriggered = 0;            /* Set by GPF_IRQHandler when INA_WARNING pin fires          */
static volatile uint8_t g_u8NP23PlayFlag      = 0;            /* Set by SysTick in Phase 1; main loop calls N_PLAY(1)      */
static volatile uint8_t g_u8NP23Play2Flag     = 0;            /* Set by SysTick in Phase 2; main loop calls N_PLAY(2)      */
static uint8_t  g_u8ProtectionChecked         = 0;            /* Allows only one protection evaluation per I2C data cycle  */
volatile uint32_t g_u32McuRunTimeSeconds = 0;                  /* MCU uptime in seconds; mirrored to eeprom_ram             */
static volatile uint16_t g_u16McuRunTimeMillis = 0;            /* Millisecond sub-counter; rolls over into seconds at 1000  */
/* I2C command flags: set by I2C0_IRQHandler, cleared and acted upon in Event_Log_Handler */
...
extern volatile uint8_t u8UPDCFlag;        /* SWDC cmd: update software debounce count  */
extern volatile uint8_t u8UPCAFlag;        /* UPCA cmd: update channel calibration data */
...
//extern volatile uint8_t u8UPUCFlag;      /* UPUC cmd: update undercurrent threshold (reserved) */

/* Asset information buffers; loaded from EEPROM and mirrored to eeprom_ram for host access */
uint8_t g_au8SerialNumber[EEPROM_SERIAL_NUMBER_SIZE] = {0}; /* Product serial number */
...
/* Application configuration: protection thresholds, countdown, power-on count, etc. */
...
/*---------------------------------------------------------------------------------------------------------*/
/*  SYS_Init                                                                                               */
/*  Enables the HIRC internal oscillator and sets the core clock (HCLK) to HCLK_CLK MHz (48 MHz).         */
/*---------------------------------------------------------------------------------------------------------*/
void SYS_Init(void)
{
    SYS_UnlockReg(); /* Unlock write-protected registers before clock configuration */
    /*---------------------------------------------------------------------------------------------------------*/
    /* Init System Clock                                                                                       */
    /*---------------------------------------------------------------------------------------------------------*/
    /* Enable HIRC clock */
...
/*  Responsibilities:                                                                                      */
/*    1. Drives the three-phase protection warning sequence (Phase 1 -> Phase 2 -> Latch):                 */
/*         - Non-NSP23 boards: directly toggles LED and buzzer GPIO pins.                                  */
/*         - NSP23 boards    : sets a flag; main loop calls N_PLAY() when I2C1 is idle.                    */
/*    2. Sets u8MonitorFlag every TIMER_MONITOR_UPDATE ms to trigger INA3221 reads.                        */
/*    3. Maintains the MCU uptime counter and syncs it to eeprom_ram.                                      */
/*  NOTE: Do NOT call any I2C-dependent function (e.g. N_PLAY) directly from this ISR.                     */
/*---------------------------------------------------------------------------------------------------------*/
...
/*  Each entry is 22 bytes; 11 entries fit in the 256-byte system EEPROM (11 * 22 = 242 bytes).            */
...
/*---------------------------------------------------------------------------------------------------------*/
/*  Save_Log_Entry                                                                                         */
/*  Captures a snapshot of the current fault state and writes it to the system EEPROM ring buffer.         */
...
/*  Triggered by a falling edge on INA_WARNING (PF.2) when the INA3221 asserts its ALERT output.          */
/*  Sets g_u8HwWarningTriggered so Protection_Handler can immediately save a log and assert PGOOD.         */
/*---------------------------------------------------------------------------------------------------------*/
void GPF_IRQHandler(void)
{
    ...
    {
        GPIO_CLR_INT_FLAG(INA_WARNING_PORT, INA_WARNING_PIN); /* Clear flag to allow future interrupts */
        g_u8HwWarningTriggered = 1; /* Signal main loop to handle the hardware warning */
    }
...
/*---------------------------------------------------------------------------------------------------------*/
/*  ProcessHardFault                                                                                       */
/*  Called when a CPU HardFault exception occurs. Resets the chip to recover automatically.                */
...
/*  Reads a setting from EEPROM. If the value is uninitialized (0xFFFF.../0x00), writes the default.      */
/*  Parameters:                                                                                            */
...
/*---------------------------------------------------------------------------------------------------------*/
/*  initial_eeprom_ram                                                                                     */
/*  Pre-populates fixed fields of the I2C slave register map (eeprom_ram) with the firmware version        */
/*  number and the board model string "M251_GPU_CP". The host reads these at 0xF0 onwards.                 */
/*---------------------------------------------------------------------------------------------------------*/
void initial_eeprom_ram(void)
{
    eeprom_ram[0xf0] = FW_version; /* Firmware version (0x01) */
...
void Peripherals_Init(void)
{
  ...
  /* Unlock protected registers */
    ...
    NSP23_RST_PORT->DOUT |= NSP23_RST_PIN;  // NSP23 RST high = device running normally
    ...
    /* I2C0 slave — handles host communication (register map at eeprom_ram, addr 0x4D) */
    I2C0_Init();
...
void Config_Load(void)
{
    ...
    // Do NOT use a single memcpy of the whole struct: struct padding would corrupt adjacent registers.
    // Copy each member individually to its defined offset in the I2C register map.
...
void Protection_Handler(void)
{
    ...
    /* g_u8ProtectionChecked so the protection logic runs exactly once per data cycle.  */
    if (u8MonitorFlag == 1)
    {
        u8MonitorFlag = 0;
    ...
    if ((g_u8GetEndFlag_1 == 1) && (g_u8GetEndFlag_0 == 1) && (g_u8ProtectionChecked == 0))
    {
        ...
        if (g_system_state != STATE_LATCHED)
        {
            ...
            //if(min_current!=0)
            {
                ...
                if (max_current > (g_AppConfig.u32OcThreshold / 10))
                {
...
/*---------------------------------------------------------------------------------------------------------*/
/*  uart_init                                                                                              */
/*  Initialises UART0 at 115200 bps for debug output. Not called in normal production flow.               */
/*---------------------------------------------------------------------------------------------------------*/
void uart_init(void)
{
    SYS_UnlockReg();
