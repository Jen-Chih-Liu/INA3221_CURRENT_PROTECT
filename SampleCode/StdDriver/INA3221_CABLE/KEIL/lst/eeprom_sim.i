# 1 "../EEPROM/eeprom_sim.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 391 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "../EEPROM/eeprom_sim.c" 2
...
# 4 "../EEPROM/eeprom_sim.c" 2
# 20 "../EEPROM/eeprom_sim.c"
static uint8_t gs_BufferPool[2][255UL];
static uint8_t gs_u8AllocatedCount = 0;
# 38 "../EEPROM/eeprom_sim.c"
...
uint32_t Init_EEPROM(EEPROM_Ctx_T *ctx, uint32_t base_addr, uint32_t data_amount, uint32_t use_pages)
{
    ...
    if(ctx->Amount_of_Data > 255UL) return 0x01;
    if(ctx->Amount_Pages > 20) return 0x02;
    if(ctx->Amount_Pages < 2) return 0x02;


    if(ctx->Flash_BaseAddr % 0x200UL != 0) return 0x04;


    if (gs_u8AllocatedCount >= 2) {
...
static void Search_Valid_Page(EEPROM_Ctx_T *ctx)
{
    ...
    {
        Page_Status_Array[i] = (uint16_t)FMC_Read(ctx->Flash_BaseAddr + (0x200UL * i));
    }
    ...
    for(i = 0; i < ctx->Amount_Pages; i++)
    {
        if(Page_Status_Array[i] != 0xFFFF)
            ctx->Current_Valid_Page = i;
    }


    if(Page_Status_Array[ctx->Current_Valid_Page] == 0xFFFF)
    {
        FMC_Write(ctx->Flash_BaseAddr + (0x200UL * ctx->Current_Valid_Page), 0xFFFF0000);
        ctx->Current_Cursor = 2;
    }
    else
    {

        temp = FMC_Read(ctx->Flash_BaseAddr + (0x200UL * ctx->Current_Valid_Page));
        ...
        else
        {
            ...
            for(i = 4; i < 0x200UL; i += 4)
            {

                temp = FMC_Read(ctx->Flash_BaseAddr + (0x200UL * ctx->Current_Valid_Page) + i);
                addr = (temp & 0xFF) >> 0UL;
...
uint32_t Read_Data(EEPROM_Ctx_T *ctx, uint8_t index, uint8_t *data)
{
    if(index >= ctx->Amount_of_Data) return 0x03;
...
uint32_t Write_Data(EEPROM_Ctx_T *ctx, uint8_t index, uint8_t data)
{
    ...
    if(index >= ctx->Amount_of_Data) return 0x03;
    if(ctx->Written_Data[index] == data) return 0;

    FMC_Enable();

    target_addr = ctx->Flash_BaseAddr + (0x200UL * ctx->Current_Valid_Page);

    if((ctx->Current_Cursor & 0x3) == 0)
    {
        temp = 0xFFFF0000 | (index << 0UL) | (data << 8UL);
        FMC_Write(target_addr + ctx->Current_Cursor, temp);
        ctx->Written_Data[index] = data;
    }
    else
    {
        temp = FMC_Read(target_addr + (ctx->Current_Cursor - 2));
        temp &= ~(0xFF0000 | 0xFF000000);
        temp |= (index << 16UL) | (data << 24UL);
        FMC_Write(target_addr + (ctx->Current_Cursor - 2), temp);
        ctx->Written_Data[index] = data;
    }

    if(ctx->Current_Cursor >= (0x200UL - 2))
    {
...
static void Manage_Next_Page(EEPROM_Ctx_T *ctx)
{
    ...
    counter = FMC_Read(ctx->Flash_BaseAddr + (0x200UL * ctx->Current_Valid_Page));
    ...
    new_page_addr = ctx->Flash_BaseAddr + (0x200UL * new_page);



    while(1)
    {
        if (i >= ctx->Amount_of_Data) { i=0; break; }
        ...
        else
        {
            ...
            counter |= (i << 16UL) | (ctx->Written_Data[i] << 24UL);
    ...
    for(j = 4; i < ctx->Amount_of_Data; i++)
    {
        ...
        {
            temp = 0xFFFF0000;
            temp |= (i << 0UL) | (ctx->Written_Data[i] << 8UL);
            data_flag = 1;
        }
        else
        {
            temp &= ~(0xFF0000 | 0xFF000000);
            temp |= (i << 16UL) | (ctx->Written_Data[i] << 24UL);
            FMC_Write(new_page_addr + j, temp);
            temp = 0;
            data_flag = 0;
            j += 4;
        }
    ...
    FMC_Erase(ctx->Flash_BaseAddr + (0x200UL * ctx->Current_Valid_Page));
    ctx->Current_Valid_Page = new_page;
}

uint16_t Get_Cycle_Counter(EEPROM_Ctx_T *ctx)
{
    return (uint16_t)FMC_Read(ctx->Flash_BaseAddr + (0x200UL * ctx->Current_Valid_Page));
}
# 312 "../EEPROM/eeprom_sim.c"
uint32_t Read_EEPROM(EEPROM_Ctx_T *ctx, uint32_t offset, uint8_t *data, uint32_t len)
{
    uint32_t i;
